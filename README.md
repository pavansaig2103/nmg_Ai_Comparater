...
> For the current single-project Vercel deployment setup, use [VERCEL_DEPLOYMENT.md](./VERCEL_DEPLOYMENT.md). The backend is now exposed through Vercel serverless functions under `/api`, and production requires a hosted PostgreSQL `DATABASE_URL`.
